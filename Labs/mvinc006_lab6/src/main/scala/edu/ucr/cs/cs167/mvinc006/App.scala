...
object App {
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      val validLines: RDD[String] = inputRDD.filter(a=> !a.startsWith("host\tlogname"))// TODO 1a: filter lines which do not start with "host\tlogname" from `inputRDD`
      val parsedLines: RDD[Array[String]] = validLines.map(m=> m.split("\t"))// TODO 1b: split each line by "\t" from `validLines` via `map`
      // Test Case: Print RDDs
      // validLines.take(1).foreach(println)
      ...
      command match {
        ...
        case "code-filter" =>
          ...
          val filteredLines: RDD[Array[String]] = parsedLines.filter(a=> a.contains(args(2)))// TODO 3: `filter` on `parsedLines` by `responseCode`
        ...
        case "time-filter" =>
          ...
          val filteredLines: RDD[Array[String]] = parsedLines.filter(a=> a(2).toLong >= from && a(2).toLong <= to)// TODO 4: `filter` on `parsedLines` by time (column 2) with `from` and `to`
          val count: Long = filteredLines.count()
        ...
        case "count-by-code" =>
          ...
          val loglinesByCode: RDD[(String, Long)] = parsedLines.map(a=> (a(5).toString, 1)) // TODO 5a: `map` on `parsedLines` by response code (column 5)
        ...
        case "sum-bytes-by-code" =>
          ...
          val loglinesByCode: RDD[(String, Long)] = parsedLines.map(a=> (a(5), a(6).toLong))// TODO 6a: `map` on `parsedLines` by response code (column 5) and bytes (column 6)
        ...
        case "avg-bytes-by-code" =>
          ...
          val loglinesByCode: RDD[(String, Long)] = parsedLines.map(a=> (a(5), a(6).toLong))// TODO 7a: `map` on `parsedLines` by response code (column 5) and bytes (column 6)
        ...
        case "top-host" =>
          ...
          val loglinesByHost: RDD[(String, Long)] = parsedLines.map(a=> (a(0), 1))// TODO 8a: `map` on `parsedLines` by host (column 0)
          val counts: RDD[(String, Long)] = loglinesByHost.reduceByKey(_+_)// TODO 8b: `reduceByKey` on `loglinesByHost`
          val sorted: RDD[(String, Long)] = counts.sortBy(_._2, false)// TODO 8c: `sortBy` on `counts`
