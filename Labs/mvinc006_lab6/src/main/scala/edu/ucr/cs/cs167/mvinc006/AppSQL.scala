    ...
    try {
      ...
      command match {
        ...
        case "count-by-code" =>
          ...
          println("Code,Count")
          val query: String = """SELECT response, COUNT(response) FROM log_lines GROUP BY response ORDER BY response"""// TODO 12: write SQL query the get the count for each `response`, results must be ordered by `response` in ascending order
